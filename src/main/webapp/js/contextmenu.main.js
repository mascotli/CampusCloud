...
function confirmEditName() {
    ...
    if (newName.length == 0) {
        ...
        /* 需要交给服务器的数据 */
        var renameData;
        var req_url = "http://localhost:8080/CampusCloud/api/v1/users/"+sessionStorage.getItem("user_username")+"/disk/"
...
function remove() {
    ...
    $.ajax({
        type: "PATCH",
        url: "http://localhost:8080/CampusCloud/api/v1/users/"+sessionStorage.getItem("user_username")+"/disk/move",
        contentType: "application/json;charset=utf-8",
...
function download() {
    ...
    var url = "http://localhost:8080/CampusCloud/api/v1/users/"+sessionStorage.getItem("user_username")+"/disk/files?token=Bearer "+sessionStorage.getItem("token")+"&files="+fileParams+"&folders="+folderParams;
