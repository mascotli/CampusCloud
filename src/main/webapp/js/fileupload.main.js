...
function addFile(e, data, marker) {
	...
	browserMD5File(data.files[0], function (err, md5) {
		...
		data.context.find(".upload-cancel").click(function() {
            ...
            $.ajax({
            	type: "DELETE",
            	url:"http://localhost:8080/CampusCloud/api/v1/users/"+sessionStorage.getItem("user_username")+"/disk/files?cancel=" + data.formData.md5,
		...
		data.context.find(".upload-pause").click(function() {
			...
			if (icon.hasClass("glyphicon-pause")) {
				...
				$.ajax({
					type: "GET",
					url:"http://localhost:8080/CampusCloud/api/v1/users/"+sessionStorage.getItem("user_username")+"/disk/files?resume=" + data.formData.md5,
