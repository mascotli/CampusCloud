...
public class OperatingFiles {
		...
		try {
			ugi.doAs(new PrivilegedExceptionAction<Void>() {
				public Void run() throws Exception {
					...
					// "hdfs://godfather.mascot.com.cn:9000/"对应的是你自己的网址
					conf.set("fs.default.name", "hdfs://godfather.mascot.com.cn:9000/");
					// conf.set("hadoop.job.ugi", "root");
					// 以下两行是支持 hdfs的追加 功能的：hdfs.append()
					conf.set("dfs.client.block.write.replace-datanode-on-failure.policy", "NEVER");
					conf.set("dfs.client.block.write.replace-datanode-on-failure.enable", "true");
					Path path = new Path("hdfs://godfather.mascot.com.cn:9000/");
	...
	public void readFile() throws IOException {
		...
		try {
			...
			// InputStreamReader(in,"GB2312"));//防止中文乱码
			// 复制到标准输出流
	...
	public void copyFile() throws IOException {
		...
		if (!(new File(localSrc)).exists()) {
			System.out.println("Error: local dir \t" + localSrc + "\t not exists.");
			return;
		}
		// hdfs路径不存在
		if (!hdfs.exists(dst)) {
			System.out.println("Error: dest dir \t" + dst.toUri() + "\t not exists.");
			return;
		}
		...
		if (hdfs.exists(new Path(dstPath))) {
			System.out.println("Warn: dest file \t" + dstPath + "\t already exists.");
		} else {
			// 本地文件上传hdfs
	...
	public void appendFile() throws IOException {
		...
		output.write(bytes);
		System.out.println("successful: append to file \t" + conf.get("fs.default.name") + fileName);
	}
